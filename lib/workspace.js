...
async function findOrCreateWorkspace({ workDir, repos, busyPaths = [], requireUnused = true }) {
  ...
  // pass the cwds of sessions that should reserve a workspace.
...
// Turn a user-supplied session/workspace name into a single safe directory
// segment: strip path separators, Windows-illegal characters, and control
// chars, then trim the leading/trailing dots and spaces Windows forbids at the
// ends of a name. Returns '' when nothing usable remains.
...
// Create (or reuse) a workspace directory with a user-chosen name under
// workDir. Unlike findOrCreateWorkspace this never auto-allocates ws-N — the
// caller wants exactly this name. Reusing an existing same-named folder is
// intentional: multiple sessions may share one work folder.
async function createNamedWorkspace({ workDir, name, repos = [], busyPaths = [] }) {
  const safe = sanitizeWorkspaceName(name);
  if (!safe) throw new Error('Invalid session name');
  const wsPath = path.join(workDir, safe);
  // Defense in depth against traversal: the sanitized name must resolve to a
  // direct child of workDir.
  if (!isInside(wsPath, workDir) || path.dirname(path.resolve(wsPath)) !== path.resolve(workDir)) {
